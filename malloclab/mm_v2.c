...
/* packing the size and allocated bit into a word */
#define PACK3(size, prev_alloc, alloc)  ((size) | (prev_alloc) | (alloc))
...
int mm_init(void)
{
    ...
    PUT(heap_listp + 3*WSIZE, PACK3(0, 0x2, 1));
    heap_listp += 2*WSIZE;
...
void mm_free(void *ptr)
{
    ...
    PUT(HDRP(ptr), PACK3(size, prev_alloc, 0));
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if (newsize < 2 * DSIZE) newsize = 2 * DSIZE;
    
    size_t prev_alloc = GET_PREV_ALLOC(HDRP(ptr));
    
    if (newsize <= oldsize) {
        if (oldsize - newsize >= 2 * DSIZE) {
            PUT(HDRP(ptr), PACK3(newsize, prev_alloc, 1));
            void *next_bp = NEXT_BLKP(ptr);
            PUT(HDRP(next_bp), PACK3(oldsize - newsize, 0x2, 0));
    ...
    if (!next_alloc && (oldsize + next_size >= newsize)) {
        ...
        if (total_avail - newsize >= 2 * DSIZE) {
            ...
            PUT(HDRP(new_next), PACK3(total_avail - newsize, 0x2, 0));
            PUT(FTRP(new_next), PACK(total_avail - newsize, 0));
            coalesce(new_next);
        } else {
            PUT(HDRP(ptr), PACK3(total_avail, prev_alloc, 1));
...
void *coalesce(void *bp)
{
    ...
    if (prev_alloc && next_alloc) {
        ...
        PUT(HDRP(PREV_BLKP(bp)), PACK3(size, GET_PREV_ALLOC(HDRP(PREV_BLKP(bp))), 0));
        bp = PREV_BLKP(bp);
        insert_node(bp);
    }
    else if (prev_alloc && !next_alloc) {
        ...
        PUT(HDRP(bp), PACK3(size, prev_alloc, 0));
        PUT(FTRP(bp), PACK(size, 0));
        insert_node(bp);
    }
    else {
...
void *place(void *bp, size_t asize)
{
    ...
    if ((csize - asize) >= (2 * DSIZE)) { 
        if (asize >= 32) { 
            ...
            PUT(HDRP(next_bp), PACK3(asize, 0, 1));
            PUT(HDRP(NEXT_BLKP(next_bp)), GET(HDRP(NEXT_BLKP(next_bp))) | 0x2);
            return next_bp;
        } else {
            PUT(HDRP(bp), PACK3(asize, prev_alloc, 1)); 
            void *next_bp = NEXT_BLKP(bp); 
            PUT(HDRP(next_bp), PACK3(csize - asize, 0x2, 0));
        ...
        PUT(HDRP(bp), PACK3(csize, prev_alloc, 1));  
        PUT(HDRP(NEXT_BLKP(bp)), GET(HDRP(NEXT_BLKP(bp))) | 0x2);
        return bp;
    }
}
...
void *extend_heap(size_t words)
{
    ...
    PUT(HDRP(bp), PACK3(size, prev_alloc, 0));
    PUT(FTRP(bp), PACK(size, 0));
    PUT(HDRP(NEXT_BLKP(bp)), PACK3(0, 0, 1));
    return coalesce(bp);
}


int get_list_idx(size_t size) {
    static const size_t size_classes[NUM_LISTS - 1] = {
        16, 24, 32, 40, 48, 64, 96, 128, 256, 512, 1024
    };
